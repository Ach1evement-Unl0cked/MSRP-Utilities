...
client.dispatcher.addInhibitor((msg) => {
  ...
  let mod = msg.guild.roles.cache.find((r) => r.id == client.config.role.mod || r.name == client.config.role.mod);
  let admin = msg.guild.roles.cache.find((r) => r.id == client.config.role.admin || r.name == client.config.role.admin);
  let ia = msg.guild.roles.cache.find((r) => r.id == client.config.role.ia || r.name == client.config.role.ia);
  let management = msg.guild.roles.cache.find((r) => r.id == client.config.role.management || r.name == client.config.role.management);
  let directive = msg.guild.roles.cache.find((a) => a.id == client.config.role.directive || a.name == client.config.role.directive);
  let support = msg.guild.roles.cache.find((r) => r.id == client.config.role.support || r.name == client.config.role.support);
  switch (msg.command.group.name) {
    case "administration":
      if (msg.member.roles.cache.has(admin.id) || msg.member.roles.cache.has(ia.id) || msg.member.roles.cache.has(directive.id) || msg.member.roles.cache.has(management.id) || client.config.ownerid == msg.member.user.id) {
        return false;
      } else {
        client.error("Invalid permissions, you must be an Administrator!", msg);
        return "Invalid permissions";
      }
      break;
    case "moderation":
      if (msg.member.roles.cache.has(mod.id) || msg.member.roles.cache.has(admin.id) || msg.member.roles.cache.has(ia.id) || msg.member.roles.cache.has(directive.id) || msg.member.roles.cache.has(management.id) || client.config.ownerid == msg.member.user.id) {
        return false;
      } else {
        client.error("Invalid permissions, you must be a Moderator!", msg);
        return "Invalid permissions";
      }
      break;
    case "ia":
      if (msg.member.roles.cache.has(ia.id) || msg.member.roles.cache.has(directive.id) || msg.member.roles.cache.has(management.id) || client.config.ownerid == msg.member.user.id) {
        return false;
      } else {
        client.error("Invalid permissions, you must be an Internal Affairs Member!!", msg);
        return "Invalid permissions";
      }
      break;
    case "management":
      if (msg.member.roles.cache.has(directive.id) || msg.member.roles.cache.has(management.id) || client.config.ownerid == msg.member.user.id) {
        return false;
      } else {
        client.error("Invalid permissions, you must be an Management member!", msg);
        return "Invalid permissions";
      }
      break;
    case "directive":
      if (msg.member.roles.cache.has(directive.id) || client.config.ownerid == msg.member.user.id) {
        return false;
      } else {
        client.error("Invalid permissions, you must be an Directive Member!!", msg);
        return "Invalid permissions";
      }
      break;
    case "support":
      if (msg.member.roles.cache.has(support.id) || client.config.ownerid == msg.member.user.id) {
        return false;
      } else {
        client.error("Invalid permissions, you must be an member of our Support Team!", msg);
        return "Invalid permissions";
      }
      break;
    default:
...
client.on("message", async (message) => {
  ...
  if (message.guild && message.channel.parentID == message.guild.channels.cache.find((ch) => ch.name == client.config.modmail.category || ch.id == client.config.modmail.category) && !message.author.bot && message.content.startsWith(client.config.prefix)) {
    guild.members.fetch();
  ...
  if (!message.author.bot && message.guild && message.channel.parentID == message.guild.channels.cache.find((ch) => ch.name == client.config.modmail.category || ch.id == client.config.modmail.category) && message.channel.type != "dm") {
...
client.on("channelDelete", async (channel) => {
  ...
  await guild.channels.cache
    ...
    .send({
      embed: {
        author: { name: "Ticked Closed", icon_url: "https://i.imgur.com/4Pfai3M.png" },
